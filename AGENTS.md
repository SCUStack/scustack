...
This file is the Codex CLI agent guide for working in this repository. Unless a task explicitly says otherwise, follow these rules for all coding, review, and documentation work.
...
- Prefer focused edits that match the existing architecture instead of broad opportunistic refactors.
...
- Avoid destructive git commands unless the task explicitly requires them.
...
When asked to implement `ISSUE-025` (College-Course cascading selector):

1. Read `ISSUE-025` in `docs/ISSUES.md` — note it is blocked by `ISSUE-024` and needs the `/api/v1/courses?college_id=` endpoint
2. Check `ISSUE-024` is complete (colleges + courses API exists)
3. Implement the full slice:
...
5. Commit with message `feat: add college-course cascading selector (ISSUE-025)` once the slice is complete
